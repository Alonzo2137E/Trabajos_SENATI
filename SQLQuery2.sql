...
--PROCEDIMIENTO ALMACENADO Especilista
...
INSERT INTO [dbo].[Especialista]
           ...
           ,[EspecialidadCodigo])
     VALUES
           (@EspecialistaCMP,@EspecialistaNombre,@EspecialistaApellido,@EspecialidadCodigo)
END TRY
BEGIN CATCH
SELECT ERROR_NUMBER(),ERROR_MESSAGE()
END CATCH
...
create proc usp_EliminarEspecialista
(
@EspecialistaCMP int
)
as
...
create proc usp_ListarEspecialistaCMP
(
@EspecialistaCMP int
)
as
...
CREATE PROC usp_ListarEspecialista
AS
BEGIN
    BEGIN TRY
        SELECT * FROM Especialista;
